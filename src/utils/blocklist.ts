...
export function isUrlOrQueryBlocked(
  ...
  // 1. Check direct domain & custom domain match using hostname boundaries
  const allBlockedDomains = [
    ...DEFAULT_BLOCKED_DOMAINS, 
    ...customBlockedDomains.map(d => d.toLowerCase().trim())
  ].filter(Boolean);

  // Extract hostname if input is a URL
  let parsedHostname = '';
  try {
    const urlString = raw.startsWith('http://') || raw.startsWith('https://') ? raw : `https://${raw}`;
  ...
  for (const domain of allBlockedDomains) {
    ...
    // A) If parsed hostname is available and has a valid TLD, perform exact domain / subdomain matching ONLY
    if (parsedHostname && parsedHostname.includes('.')) {
      if (parsedHostname === domain || parsedHostname.endsWith('.' + domain)) {
        return {
          blocked: true,
    ...
    // B) For non-URL search queries or raw strings without dots, use word-boundary domain matching
    const domainRegex = new RegExp(`(?:^|[^a-z0-9.-])${domain.replace(/\./g, '\\.')}(?:[^a-z0-9.-]|$)`, 'i');
    if (domainRegex.test(raw) || domainRegex.test(normalized)) {
      return {
        blocked: true,
        reason: 'محاولة الوصول لنطاق محظور لحماية البصر'
      };
  ...
  for (const keyword of EXPLICIT_KEYWORDS) {
    ...
    if (isArabic) {
      ...
      const isSpacedObfuscation = /[\s._\-+/\\,*#@!$%^&()=[\]{}|;:'"<>?]+/.test(raw);
      const matchesDeSpaced = isSpacedObfuscation && regexNorm.test(deSpaced);
