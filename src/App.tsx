...
function MainApp() {
  ...
  // Continuously persist measured daily stats to historical history log
  useEffect(() => {
    const todayStr = getLocalFormattedDate();
    const confirmedCount = prayers.filter(p => p.confirmed).length;
    recordStatsForDay(todayStr, {
      confirmedPrayers: confirmedCount,
